...
async function onSubmit(event: FormSubmitEvent<typeof state>) {
    ...
    try {
        ...
        // นำทางไปหน้า Aduser หลังจากบันทึกสำเร็จ
        router.push('/Aduser');
    } catch (error: any) {
        toast.add({
        ...
        <div class="w-full max-w-md bg-white p-8 rounded-xl shadow-md relative" style="z-index: 0;">
            <UForm :validate="validate" :state="state" class="space-y-4" @submit="onSubmit">
                ...
                <div class="flex justify-end gap-4 p-4">
                    <nuxt-link to="/Aduser">
                        <UButton type="button" class="bg-gray-500 hover:bg-gray-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition duration-200" @click="$emit('cancel')">
                            Cancel
                        </UButton>
                    </nuxt-link>
